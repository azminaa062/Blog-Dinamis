...
$tag = mysqli_fetch_assoc(mysqli_query($conn,
    "SELECT * FROM tags WHERE slug='$slug_safe' LIMIT 1"
));

if (!$tag) {
    $pageTitle = 'Tag Tidak Ditemukan';
    include 'templates/frontend_header.php';
    echo '<div class="bd-layout"><main class="bd-feed"><div class="empty-state"><div class="emoji">🏷️</div><p>Tag tidak ditemukan.</p><a href="/blog_dinamis/index.php" style="color:#1a8917;font-family:sans-serif;">← Kembali ke Beranda</a></div></main></div>';
...
$data = mysqli_query($conn, "
    ...
    WHERE t.slug = '$slug_safe' AND a.status = 'publish'
    ORDER BY a.id DESC
");
...
<div class="bd-layout">
    <main class="bd-feed">
        ...
        <div class="page-header">
            <div class="page-header-eyebrow">Tag</div>
            <h1 class="page-header-title">#<?= htmlspecialchars($tag['nama_tag']); ?></h1>
            <p class="page-header-desc"><?= $data ? mysqli_num_rows($data) : 0; ?> artikel dengan tag ini</p>
        </div>
